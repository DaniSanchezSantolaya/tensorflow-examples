...
def create_dataset(source_data, num_tuples = 500, batch_size = 128, fraction_same = 0.2):
    ...
    for i in range(num_tuples):
        x1, x2, labels = source_data.next_batch(batch_size, fraction_same)
...
class DataSet(object):
  ...
  def next_batch(self, batch_size, fraction_same = 0.2):
    ...
    n_same = int(batch_size * fraction_same)
    n_different = batch_size - n_same
    ...
    x1 = self._images[start:end]
    x1_class = self._labels[start:end]
    x1_class = np.argmax(x1_class, axis=1)


    #Create a list of indexes where we are going to have the same class, and another list for different class
    indices = range(batch_size)
    random.shuffle(indices)
    same_indices = indices[0:n_same]
    diff_indices = indices[n_same:batch_size]
    #Group images by index
    labels_class = np.argmax(self._labels, axis=1)
    class_indices = [np.where(labels_class == i)[0] for i in range(10)] 


    for i in range(batch_size):
        if i in same_indices: #image from the same class
            c = x1_class[i]  
        else: #Sample a random class different of the image class
            classes = range(10)
            classes.remove(x1_class[i])
            c = random.choice(classes)
        #Obtain index of image of class c
        while True:
            im_class_index = random.randint(0, len(class_indices[c])-1)
            im_index = class_indices[c][im_class_index]
            x2[i] = self._images[im_index]
            #if same image repeat again
            if (x1[i]!=x2[i]).any(): 
                break
            else:
                print('same')
        
    labels[same_indices] = 1
