...
class MLP(object):
  ...
  def inference(self, x):
    ...
    #input_x = tf.placeholder(tf.float32, shape=(batch_size, tf.shape(x)[1])) 
    X = x
    for idx, val in enumerate(self.n_hidden):
        with tf.variable_scope("hidden_"+str(idx)):
            W = tf.get_variable("weights", (X.get_shape()[1], val), initializer=self.weight_initializer, regularizer=self.weight_regularizer)
            b = tf.get_variable("bias", [val], initializer=tf.constant_initializer(0.0))
            X_preact = tf.matmul(X, W) + b
            X_act = self.activation_fn(X_preact)
            X_dropout = tf.cond(self.is_training, lambda: tf.nn.dropout(X_act, (1-self.dropout_rate)), lambda: X_act)
            X = X_dropout
            #Add summaries
            tf.histogram_summary('Weights_layer_' + str(idx), W)
            tf.histogram_summary('Bias_layer_' + str(idx), b)
            tf.histogram_summary('pre-activations_layer_' + str(idx), X_preact)
            tf.histogram_summary('post-activations_' + str(idx), X_act)
            tf.histogram_summary('dropout-activations_' + str(idx), X_dropout)
            
    #output layer (without non-linearity)
    with tf.variable_scope("output"):
        W = tf.get_variable("weights", (X.get_shape()[1], self.n_classes), initializer=self.weight_initializer, regularizer=self.weight_regularizer)
        b = tf.get_variable("bias", (self.n_classes,), initializer=tf.constant_initializer(0.0))
        logits = tf.matmul(X, W) + b
        tf.histogram_summary('Weights_output', W)
        tf.histogram_summary('Bias_output', b)
